...
index_mask = [5, 40, 15, 41]

tab20 = mpl.colormaps['tab20']

ax[0].bar(np.arange(4)-0.2, eta_f[index_mask], width=0.2, label=r'$\eta_f$', color='k')
ax[0].bar(np.arange(4), eta_w[index_mask], width=0.2, label=r'$\eta_w$', color=tab20(0))
ax[0].bar(np.arange(4)+0.2, eta_nl[index_mask], width=0.2, label=r'$\eta_{nl}$', color=tab20(2))
ax[0].set_ylim([0,1.1])
ax[0].axvline(1.5, c='grey')
ax[0].set_title(r'(A)', loc='left')
ax[0].set_xticks(np.arange(4), [r'Staggered', r'Aligned', r'Staggered', r'Aligned'])
ax[0].text(0, 1.1, r'H1000-C5-G4', ha='left', va='bottom')
ax[0].text(2, 1.1, r'H500-C5-G4', ha='left', va='bottom')
ax[0].legend(loc='upper center', bbox_to_anchor=(0.5, -0.2), ncol=3)

ax[1].bar(np.arange(4)-0.2, total_loss[index_mask], width=0.2, label=r'$P_{farm}/P_{Betz}$', color='k')
ax[1].bar(np.arange(4), 1-tsl[index_mask], width=0.2, label=r'$1-\Pi_T$', color=tab20(0))
ax[1].bar(np.arange(4)+0.2, 1-fsl[index_mask], width=0.2, label=r'$1-\Pi_F$', color=tab20(2))
ax[1].set_ylim([0,1.1])
ax[1].axvline(1.5, c='grey')
ax[1].set_title(r'(B)', loc='left')
ax[1].set_xticks(np.arange(4), [r'Staggered', r'Aligned', r'Staggered', r'Aligned'])
ax[1].text(0, 1.1, r'H1000-C5-G4', ha='left', va='bottom')
ax[1].text(2, 1.1, r'H500-C5-G4', ha='left', va='bottom')
